...
def write_model(input_base_path, model_size):
    ...
    for layer_i in tqdm(range(n_layers)):
        if model_size == "7B":
            state_dict |= {
                ...
                f"layers.{layer_i}.ffn_norm.weight": loaded[
            ...
            state_dict |= {
                ...
                f"layers.{layer_i}.ffn_norm.weight": loaded[0][
                    f"layers.{layer_i}.ffn_norm.weight"
                ],
            }
            wq = torch.cat(
            ...
            state_dict[f"layers.{layer_i}.attention.wqk.weight"] = torch.cat(
                (wq, wk), dim=0
            )
    ...
    state_dict_2 = {
        key.replace(
            f'layers.{int(key.split(".")[1])}.', f'layers.{int(key.split(".")[1])-30}.'
        )
        if "layers." in key
        else key: value
        for key, value in state_dict.items()
    ...
    torch.save(state_dict_1, "./weights/pp2/merged_0.pth")
    torch.save(state_dict_2, "./weights/pp2/merged_1.pth")
